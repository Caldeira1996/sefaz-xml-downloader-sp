...
function convertPfxToPem(pfxBuffer, passphrase, outputDir) {
  // Decodifica o buffer PFX (DER) para ASN.1
  const p12Asn1 = forge.asn1.fromDer(pfxBuffer.toString('binary'));
  // Extrai a estrutura PKCS#12 com a senha
  ...
  // Extrai cadeia de certificados intermediários (se houver)
  const caCerts = p12.getBags({ bagType: forge.pki.oids.certBag })[forge.pki.oids.certBag]
    .slice(1) // pula o primeiro que é o certificado principal
